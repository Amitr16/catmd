#!/usr/bin/env python3
"""
A/B benchmark — gpt-4o-mini WITHOUT vs WITH pinned-facts header.

Same 15 recall-heavy prompts, same model, same params. Difference:
the WITH variant runs the heuristic fact-retrieval and pins the top 7
facts at the END of the system prompt. Measures the lift the new
factRetrieval.ts service gives over the baseline.

Run:
    OPENAI_API_KEY=sk-... python scripts/bench-recall-pinned.py
"""
import json
import os
import re
...
OUT_DIR = Path(__file__).parent / "bench-output-pinned"
OUT_DIR.mkdir(exist_ok=True)

# Same system prompt as bench-recall.py
...
# ----------------------------------------------------------------------
# Pinned-facts header — same algorithm as src/services/factRetrieval.ts
# but inlined for the test. Picks 7 facts via keyword + intent matching
# and renders them at the END of the prompt.
# ----------------------------------------------------------------------
...
CATCH_ME_UP_RE = re.compile(r"\b(catch me up|catch up|what'?s (?:up|new|happening|going on)|anything new|fill me in|update me|the gossip)\b", re.I)
...
def select_facts(user_msg, limit=None):
    if limit is None:
...
# ----------------------------------------------------------------------
# Same recall prompts as bench-recall.py
# ----------------------------------------------------------------------
...
def run():
    print(f"Running {len(PROMPTS)} recall prompts × 2 variants (baseline vs pinned) = {len(PROMPTS) * 2} calls...\n")
    results = []
    for i, p in enumerate(PROMPTS, 1):
        print(f"[{i:>2}/{len(PROMPTS)} | {p['tier']}] {p['user'][:60]}")
        # Build the WITH-pinned system prompt
        picked = select_facts(p["user"])
        with_facts_system = BASE_SYSTEM + render_facts_header(picked) + CLOSING
        baseline_system = BASE_SYSTEM + CLOSING
        with ThreadPoolExecutor(max_workers=2) as pool:
            f_base = pool.submit(call_oa, baseline_system, p["user"])
            f_pin = pool.submit(call_oa, with_facts_system, p["user"])
            base, pin = f_base.result(), f_pin.result()
        base_score = score(base.get("content", ""), p)
        pin_score = score(pin.get("content", ""), p)
        results.append({
            "i": i, "prompt": p,
            "baseline": {**base, "score": base_score},
            "pinned": {**pin, "score": pin_score, "picked_tiers": [f["tier"] for f in picked]},
        })
        print(f"   BASE [{base_score['hit_count']}/{base_score['hit_total']}]: {(base.get('content') or 'ERR')[:100].replace(chr(10), ' ')}")
        print(f"   PIN  [{pin_score['hit_count']}/{pin_score['hit_total']}]: {(pin.get('content') or 'ERR')[:100].replace(chr(10), ' ')}")
        print(f"        picked tiers: {','.join(set(f['tier'] for f in picked))}")
        print()
    ...
    summary = {"baseline": agg("baseline"), "pinned": agg("pinned")}
    (OUT_DIR / "summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    sm = ["# Pinned-facts A/B — gpt-4o-mini, baseline vs pinned-relevant-facts header\n\n"]
    if summary["baseline"] and summary["pinned"]:
        b, p = summary["baseline"], summary["pinned"]
        delta = p["recall_pct"] - b["recall_pct"]
        sm.append("| Metric | Baseline | Pinned | Δ |\n|---|---|---|---|\n")
        sm.append(f"| Recall hit-rate | {b['recall_pct']}% | **{p['recall_pct']}%** | **{'+' if delta >= 0 else ''}{delta:.1f}** |\n")
        sm.append(f"| Hallucinations | {b['hallucinations']} | {p['hallucinations']} | — |\n\n")
        sm.append("## Recall by tier\n\n| Tier | Baseline | Pinned | Δ |\n|---|---|---|---|\n")
        for t in sorted(set(b["by_tier_pct"]) | set(p["by_tier_pct"])):
            bv = b["by_tier_pct"].get(t, 0)
            pv = p["by_tier_pct"].get(t, 0)
            d = pv - bv
            sm.append(f"| {t} | {bv}% | {pv}% | **{'+' if d >= 0 else ''}{d:.1f}** |\n")
