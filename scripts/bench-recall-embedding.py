#!/usr/bin/env python3
"""
A/B/C benchmark — gpt-4o-mini WITHOUT pinning vs WITH KEYWORD pinning vs
WITH EMBEDDING pinning.

Same 15 recall-heavy prompts, same model, same params. Three variants:
  - BASE      : no pinned-facts header at all
  - KEYWORD   : pin top-7 facts ranked by the keyword + intent heuristic
                (what production ships today via factRetrieval.ts's
                selectRelevantFacts)
  - EMBEDDING : pin top-7 facts ranked by cosine similarity over
                text-embedding-3-small (what the new
                selectRelevantFactsByEmbedding does behind the
                EXPO_PUBLIC_FACT_SCORER=embedding flag)

The fixture (BASE_SYSTEM, ALL_FACTS, PROMPTS, scorer) is copied from
bench-recall-pinned.py so the comparison is apples-to-apples — only the
ranker changes.
...
Output:
    scripts/bench-output-embedding/summary.md
    scripts/bench-output-embedding/summary.json
    scripts/bench-output-embedding/results.json
"""
import json
import math
import os
...
OUT_DIR = Path(__file__).parent / "bench-output-embedding"
OUT_DIR.mkdir(exist_ok=True)

# ────────────────────────────────────────────────────────────────────────
# Fixture (copied verbatim from bench-recall-pinned.py)
# ────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────
# KEYWORD scorer (mirror of factRetrieval.ts:scoreFact + selectRelevantFacts)
# ────────────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────────────
# EMBEDDING scorer (cosine over text-embedding-3-small) — mirrors
# selectRelevantFactsByEmbedding in src/services/factRetrieval.ts
# ────────────────────────────────────────────────────────────────────────

EMBED_MODEL = "text-embedding-3-small"
EMBED_DIM = 1536

# Per-process cache so each fact text is embedded once across all 15 prompts.
# Keyed by `[<tier>] <text>` to match the production scorer's prefix.
_embed_cache = {}
...
def embed_batch(inputs):
    """Single batched embedding call — much cheaper than N parallel
    requests. Returns a list of vectors aligned to `inputs`. Caches
    per-input."""
...
def select_facts_embedding(user_msg, limit=None):
    ...
    # Embed the user message + every fact in a single batched API call
    # (the user message goes first; we slice the vectors out by index).
...
# ────────────────────────────────────────────────────────────────────────
# Prompt assembly + chat call + scoring (shared)
# ────────────────────────────────────────────────────────────────────────
...
def run():
    print(f"Running {len(PROMPTS)} recall prompts × 3 variants (base / keyword / embedding)")
    print(f"  = {len(PROMPTS) * 3} chat-completion calls")
    print(f"  + {len(ALL_FACTS) + len(PROMPTS)} embedding-API calls (cached, 1 batched HTTPS round-trip)\n")
    ...
    for i, p in enumerate(PROMPTS, 1):
        ...
        # Three calls in parallel — the OpenAI rate limits comfortably allow this.
        ...
        # Show which facts each ranker picked when they DIFFER
    ...
    # ── Markdown table ────────────────────────────────────────────────
    sm = ["# Embedding vs Keyword fact-pinning — gpt-4o-mini\n\n"]
    if summary["baseline"] and summary["keyword"] and summary["embedding"]:
        b, k, e = summary["baseline"], summary["keyword"], summary["embedding"]
        sm.append("| Metric | Baseline (no pinning) | Keyword pinning | Embedding pinning | Δ vs keyword |\n")
        sm.append("|---|---|---|---|---|\n")
        ...
        sm.append("| Tier | Baseline | Keyword | Embedding | Δ vs keyword |\n|---|---|---|---|---|\n")
        all_tiers = sorted(set(b["by_tier_pct"]) | set(k["by_tier_pct"]) | set(e["by_tier_pct"]))
        for t in all_tiers:
